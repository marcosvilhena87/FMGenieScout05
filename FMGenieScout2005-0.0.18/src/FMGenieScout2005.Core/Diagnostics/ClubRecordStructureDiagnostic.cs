...
public sealed class ClubRecordStructureDiagnostic
{
    ...
    [
        ...
        ("Vitória (BA)",1,340,["EC Vitória","Vitória (BA)","Vitória"]),
    ...
    public async Task<ClubRecordStructureReport> AnalyzeAsync(
        string inputFile,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        List<ClubStructureRecord> records = [];
        ...
        List<StructureSignatureSummary> signatures = records
            .Where(x => x.Status == "CONFIRMADO")
            .GroupBy(x => x.Variant)
            .OrderByDescending(x => x.Count())
            .ThenBy(x => x.Key)
            .Select(g => new StructureSignatureSummary(
                g.Key,
                g.Count(),
        ...
        var report = new ClubRecordStructureReport(
            source, data.LongLength, sha, records, signatures, output, DateTimeOffset.UtcNow);

        progress?.Report("Gravando relatórios...");
        await File.WriteAllTextAsync(Path.Combine(output, "club-structure-report.txt"), FormatReport(report), new UTF8Encoding(true), cancellationToken).ConfigureAwait(false);
        await File.WriteAllTextAsync(Path.Combine(output, "parsed-clubs.csv"), FormatVariantsCsv(report), new UTF8Encoding(true), cancellationToken).ConfigureAwait(false);
        await File.WriteAllTextAsync(Path.Combine(output, "known-club-cross-validation.csv"), FormatValidationCsv(report), new UTF8Encoding(true), cancellationToken).ConfigureAwait(false);
        await File.WriteAllTextAsync(Path.Combine(output, "club-field-distributions.csv"), FormatSignaturesCsv(report), new UTF8Encoding(true), cancellationToken).ConfigureAwait(false);
        await File.WriteAllTextAsync(Path.Combine(output, "club-structure-contexts.txt"), FormatContexts(report), new UTF8Encoding(true), cancellationToken).ConfigureAwait(false);
        await File.WriteAllTextAsync(Path.Combine(output, "victoria-investigation.txt"), FormatVictoriaInvestigation(data, report), new UTF8Encoding(true), cancellationToken).ConfigureAwait(false);
        progress?.Report("Diagnóstico estrutural concluído.");
        return report;
    }
    ...
    private static ClubStructureRecord FindClub(byte[] data, string displayName, int division, uint expectedId, string[] terms, IReadOnlyList<int> occurrences)
    {
        ...
        return new ClubStructureRecord(
            displayName,
            division,
            expectedId,
            expectedId,
    ...
    private static bool IsPlausibleName(string value)
    {
        ...
        foreach (char ch in value)
        {
            ...
            if (char.IsLetter(ch))
            {
                ...
                if (category is not (UnicodeCategory.UppercaseLetter or UnicodeCategory.LowercaseLetter or UnicodeCategory.TitlecaseLetter or UnicodeCategory.ModifierLetter or UnicodeCategory.OtherLetter)) return false;
                // FM 2005 names in this database are Latin-based; reject unrelated scripts caused by misalignment.
                if (ch > '\u024F') return false;
                letters++;
                continue;
            }
            if (!(char.IsWhiteSpace(ch) || char.IsDigit(ch) || ch is '.' or ',' or '-' or '\'' or '(' or ')' or '&' or '/')) return false;
        }
        return letters >= 2;
    }

    private static ClubStructureRecord Missing(string displayName, int division, uint id, string status) =>
    ...
    public static string FormatReport(ClubRecordStructureReport report)
    {
        var sb = new StringBuilder();
        sb.AppendLine("FM Genie Scout 2005 — ClubRecordStructureDiagnostic 0.0.16");
        ...
        sb.AppendLine();
        sb.AppendLine("Arquivos gerados: club-structure-report.txt, parsed-clubs.csv, known-club-cross-validation.csv, club-field-distributions.csv e club-structure-contexts.txt e victoria-investigation.txt.");
        sb.AppendLine("O arquivo de origem não foi modificado.");
        return sb.ToString();
    }

    private static string FormatVariantsCsv(ClubRecordStructureReport report)
    {
        var sb = new StringBuilder("display_name,division,expected_club_id,found_club_id,club_id_offset,club_record_index_offset,club_record_index,full_name,short_name,name_offset,distance_name_to_id,variant,separator,unknown_value_1,unknown_value_2,status,confidence,context_hex\r\n");
        foreach (ClubStructureRecord c in report.Clubs)
            sb.AppendLine($"{Csv(c.DisplayName)},{c.Division},{c.ExpectedClubId},{Value(c.FoundClubId)},{Offset(c.ClubIdOffset)},{Offset(c.ClubRecordIndexOffset)},{Value(c.ClubRecordIndex)},{Csv(c.FullName ?? string.Empty)},{Csv(c.ShortName ?? string.Empty)},{Offset(c.NameOffset)},{Value(c.DistanceNameToId)},{c.Variant},{Value(c.Separator)},{Value(c.UnknownValue1)},{Value(c.UnknownValue2)},{c.Status},{c.Confidence},{c.ContextHex}");
        return sb.ToString();
    }

    private static string FormatValidationCsv(ClubRecordStructureReport report)
    {
    ...
    private static string FormatSignaturesCsv(ClubRecordStructureReport report)
    {
    ...
    private static string FormatContexts(ClubRecordStructureReport report)
    {
        var sb = new StringBuilder();
        foreach (ClubStructureRecord c in report.Clubs)
        {
            ...
            sb.AppendLine($"  id_offset={Offset(c.ClubIdOffset)} local={Value(c.ClubRecordIndex)} variant={c.Variant} separator={Value(c.Separator)} unknown1={Value(c.UnknownValue1)} unknown2={Value(c.UnknownValue2)}");
    ...
    private static string FormatVictoriaInvestigation(byte[] data, ClubRecordStructureReport report)
    {
        var sb = new StringBuilder();
        sb.AppendLine("FM Genie Scout 2005 — investigação Vitória 0.0.16");
        sb.AppendLine(new string('=', 72));
        sb.AppendLine("IDs conhecidos:");
        sb.AppendLine("  Vitória (BA): 340");
        sb.AppendLine("  Vitória (ES): 313273");
        sb.AppendLine();
        foreach (uint id in new uint[] { 340, 313273 })
        {
            byte[] pattern = BitConverter.GetBytes(id);
            List<int> hits = FindAll(data, pattern).ToList();
            sb.AppendLine($"ClubId {id}: ocorrências={hits.Count}");
            foreach (int offset in hits.Take(12))
            {
                uint? index = offset >= 4 ? BinaryPrimitives.ReadUInt32LittleEndian(data.AsSpan(offset - 4, 4)) : null;
                sb.AppendLine($"  offset=0x{offset:X8} index={Value(index)} contexto={HexContext(data, offset, 128)}");
            }
            sb.AppendLine();
        }
        ClubStructureRecord? ba = report.Clubs.FirstOrDefault(x => x.ExpectedClubId == 340);
        sb.AppendLine("Validação Vitória (BA):");
        sb.AppendLine(ba is null
            ? "  registro de validação ausente"
            : $"  status={ba.Status} confidence={ba.Confidence} found={Value(ba.FoundClubId)} offset={Offset(ba.ClubIdOffset)} full={ba.FullName ?? "-"} short={ba.ShortName ?? "-"}");
        sb.AppendLine();
        sb.AppendLine("O ID 313273 é tratado somente como Vitória (ES) e não participa da lista da Série A.");
        return sb.ToString();
    }
